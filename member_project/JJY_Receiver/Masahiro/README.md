...
全体シミュレーションの波形を下図に示します。出力はAC結合してスピーカーに接続しています。スピーカーは８Ωの抵抗として模擬しています。発振器をつなぐと長時間のシミュレーションで誤差が蓄積して、シミュレーションが発散することが確認できているので、信号源を接続してシミュレーションしています。

入力波形には40uVppの信号を入力しています。出力波形には約800mVのDCオフセットに100mVpp程度の信号が出力されていることが確認できます。出力波形の立ち上がりに5ms程度掛かるのは、AC結合用の100uFのキャパシタによる影響である。

![top_schema](image/top_schema.png)

入力波形

![top_vin](image/top_vin.png)

出力波形

![top_vout](image/top_vout.png)


#　レイアウト

レイアウトは下図のようになっています。信号は左下から入り、アンプ、ギルバートセルの縦積みを通って、右側のバッファに接続されます。バッファは大電流を必要とするので、バッファと他の回路は電源を分けています。また、気休めとして電源の安定化のためにM1とM2を使ったMIMキャパシタで回路を取り囲むようにしています。
