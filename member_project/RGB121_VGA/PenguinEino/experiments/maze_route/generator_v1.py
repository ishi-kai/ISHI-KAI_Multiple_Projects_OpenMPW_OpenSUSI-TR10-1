...
DESIGN=ROOT/'experiments/maze_route'
...
def main():
 verify();st=settings();out=DESIGN/'build';out.mkdir(exist_ok=True)
 ...
 for index in st['remove_shape_indices']:
  ...
  top.shapes(layer).erase(matches[0]);removed.append([tag,*coords])
 ...
 for inst,pin,x,y in pins[st['target']]:
  n=l2n.probe_net(reg['M2'],db.Point(u(x),u(y)));assert n is not None
  if n not in own:own.append(n)
 ...
 for net,pp in pins.items():
  ...
  for inst,pin,x,y in pp:
   metal=next(r[0] for r in lef[types[inst]]['pins'][pin]['rects'] if r[0] in ('METAL1','METAL2'))
   n=l2n.probe_net(reg[{'METAL1':'M1','METAL2':'M2'}[metal]],db.Point(u(x),u(y)))
   assert n not in own,(net,inst,pin,'target component still shorted after removal')
 ...
 dest=out/'candidate.gds';ly.write(str(dest));assert sha(source)==st['source_sha256']
 manifest={'source_gds':st['source_gds'],'source_sha256':sha(source),'candidate_sha256':sha(dest),
  ...
  'removed_boxes':removed,'added_boxes':added,'path_nodes':len(path),'vias':via_count,
