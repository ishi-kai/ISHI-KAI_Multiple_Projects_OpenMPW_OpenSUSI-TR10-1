#!/usr/bin/env python3
"""Grid+power local repair, derived from design-owned maze_route.py.

Preserve that frozen script. Like route_core_escape.py, admit equality at
minimum spacing by making the forbidden raster's boundary one DBU open.
Wire widths, actual clearances and pinned upstream rules are unchanged.
Official full-GDS DRC/LVS remain mandatory.
"""
import ast
import argparse
import hashlib
...
def main():
 ...
 if st.get('m1_spacing_mode','all_wide')=='local_wide':
  # Same morphological definition as the pinned 02_Device.drc M1W.
  wide_m1=reg['M1'].sized(-u(rules.M1_WIDE_MIN/2)).merged().sized(u(rules.M1_WIDE_MIN/2)).merged()
  gaps['M1']=rules.M1_SPACE_MIN
 def clearance(region, distance): return region.sized(u(distance)-1)
 allow=np.stack([1-raster(clearance(foreign[n],gaps[n]+widths[n]/2)) for n in ('M1','M2')])
 if not wide_m1.is_empty():allow[0] &= 1-raster(clearance(wide_m1,rules.M1_WIDE_SPACE_MIN+widths['M1']/2))
 vp=rules.V1_CUT/2
 co_gap,_=deck_limit(ROOT/'tools/TR-1um/libs.tech/klayout/tech/drc/run.drc','V1.CO')
 forbidden=(clearance(foreign['M1'],gaps['M1']+vp+rules.V1_ENC_M1)+
  clearance(foreign['M2'],gaps['M2']+vp+rules.V1_ENC_M2)+
  clearance(reg['GC']+reg['GR'],vp+rules.V1_GA_SPACE_MIN)+
  clearance(reg['CO'],vp+co_gap)+clearance(reg['V1'],vp+rules.V1_SPACE_MIN))
 if not wide_m1.is_empty():forbidden+=clearance(wide_m1,rules.M1_WIDE_SPACE_MIN+vp+rules.V1_ENC_M1)
 ...
 # Explicit, orthogonal terminal-center extensions for sub-grid offsets.
 # These are trial geometry: independent connectivity and official DRC still gate adoption.
 for tag,xa,ya,xb,yb in st.get('terminal_extensions',[]):
  assert tag in widths and (xa==xb or ya==yb)
  hw=widths[tag]/2
  rect(tag,min(xa,xb)-hw,min(ya,yb)-hw,max(xa,xb)+hw,max(ya,yb)+hw)
