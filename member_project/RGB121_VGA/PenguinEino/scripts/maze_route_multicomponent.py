...
def main():
    ...
    # Explicit same-net junction fills may be needed where separate route steps
    # leave a sub-rule gap between their own via pads. Full extraction below
    # rejects any foreign-net contact; official DRC remains the adoption gate.
    for tag,xa,ya,xb,yb in st.get('post_route_boxes',[]):
        assert tag in ('M1','M2') and xa<xb and ya<yb
        addrect(tag,xa,ya,xb,yb)
    reg,l2n,groups,owned=reextract()
