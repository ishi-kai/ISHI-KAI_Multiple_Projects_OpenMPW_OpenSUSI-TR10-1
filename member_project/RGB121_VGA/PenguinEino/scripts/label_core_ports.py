#!/usr/bin/env python3
"""Add top-cell SPICE port text labels without changing core geometry."""
...
def main() -> int:
    ...
    if len(expected) != 12 or len(set(expected)) != len(expected):
        raise RuntimeError("expected exactly 12 unique core ports")
    ...
    if {s["port"] for s in label_specs} != set(expected) or len(label_specs) != len(expected):
        raise RuntimeError("port label mapping does not cover exactly the expected 12 ports")
    ...
    if before_labels or after_labels != expected_label_rows:
        raise RuntimeError(f"top text-label delta is not exactly the expected 12 ports: {after_labels}")
