#!/usr/bin/env python3
"""Audit adopted grid+power core, including the pre-repair ancestor and final ports."""
import argparse
import ast
import hashlib
import json
...
def main():
    ...
    assert audit['actual_pin_shapes_labeled']==audit['placement_signal_pin_count']==696
    ...
    assert set(before)==set(after) and len(after)==8
    assert before['vdd']==after['vdd'] and before['vss']==after['vss']
    route_names=[r['net'] for r in m['routes']]
    assert len(route_names)==6 and set(route_names)==set(after)-{'vdd','vss'}
    ...
    # Audit all repair and escape edits against the original routed ancestor,
    # not just the final label-to-escape delta. The unchanged independent
    # SPICE was generated from this placement and the adopted gate netlist.
    base=ROOT/'experiments/a_power_flex4'
    ancestor=base/'build/diagnostic_compacted.gds'
    al=db.Layout();al.read(str(ancestor));ac=al.cell(st['top'])
    assert inst_signature(ac)==inst_signature(dc)
    assert non_top_cell_inventory(al,ac.name)==non_top_cell_inventory(dl,dc.name)
    ar=region_map(al,ac)
    all_changed=[list(k) for k in sorted(ar.keys()|dr.keys())
                 if not (ar.get(k,db.Region())^dr.get(k,db.Region())).is_empty()]
    assert all(tuple(k) in (rules.M1,rules.M2,rules.V1,rules.M1_PIN,rules.M2_PIN) for k in all_changed)
    initial=connectivity(al,ac,pins,pin_layers)
    assert len(initial['pairs'])==2 and not initial['opens'] and not initial['missing']
    assert new['power_component_counts']==initial['power_component_counts']=={'vss':1,'vdd':1}
    assert new['rail_signal_nets']==initial['rail_signal_nets']=={'vss':[],'vdd':[]}
    adopted=ROOT/'experiments/a_metal_g_power'
    assert all(sha(base/f)==sha(adopted/f) for f in
               ('ishi_vga_core.v','ishi_logo.v','out/ishi_vga_core_pnr.v','tests/expected_frame.hex'))
    functional=adopted/'build/verification.json';fr=json.loads(functional.read_text())
    assert fr['result']=='PASS' and all(sha(ROOT/p)==h for p,h in fr['sha256'].items())
    assert a.reference.resolve()==base/'build/ishi_vga_core.spice'
    assert '1\'h' not in a.reference.read_text() and '1\'b' not in a.reference.read_text()
    ...
    paths += [ancestor,base/'layout/placement.json',base/'out/ishi_vga_core_pnr.v',
              base/'build/reference.log',base/'build/sta.log',functional,
              base/'ishi_vga_core.v',base/'ishi_logo.v',base/'tests/expected_frame.hex']
    paths += [ROOT/'experiments'/n/'build/manifest.json' for n in ('a_power_fix066','a_power_fix172eq')]
    paths+=sorted((ROOT/'tools/TR-1um/libs.tech/klayout/tech/lvs').glob('*.lvs'))
    result={'status':'ACCEPTED','scope':'six core signal escapes, not pad/frame integration',
        'source_sha256':sha(source),'candidate_sha256':sha(dest),'signal_pins':696,'short_pairs':0,'open_nets':0,
    ...
    result.update(ancestor_short_pairs=2,ancestor_to_final_only_routing_changed=True,
                  adopted_artwork_and_netlist_unchanged=True,ancestor_to_final_changed_layers=all_changed,
                  limitations=['core only; organizer frame/pads/ESD not integrated',
                               'one CLK BUFTH Floating SG mask warning remains',
                               'STA uses cell delays and ideal clock, no PEX/PVT or hardware validation'])
