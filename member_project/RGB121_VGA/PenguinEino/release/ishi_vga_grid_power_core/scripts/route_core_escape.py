...
def main():
    ...
    # DRC minimum spacing permits equality. A closed raster of a closed
    # dilation would reject that legal boundary (notably adjacent 3.4 um
    # standard-cell M2 pins at exactly 2 um spacing). Subtract one DBU only
    # from exclusion growth; route dimensions themselves are unchanged.
    def clearance(region, distance):
        return region.sized(u(distance) - 1)
    ...
    for index, route in enumerate(st['ports']):
        ...
        allow = np.stack([1-raster(clearance(foreign[t], gap[t]+widths[t]/2)) for t in ('M1','M2')])
        allow[0] &= 1-raster(clearance(wide, rules.M1_WIDE_SPACE_MIN+widths['M1']/2))
        forbidden = (clearance(foreign['M1'], gap['M1']+vp+rules.V1_ENC_M1) +
            clearance(foreign['M2'], gap['M2']+vp+rules.V1_ENC_M2) +
            clearance(reg['GC']+reg['GR'], vp+rules.V1_GA_SPACE_MIN) +
            clearance(reg['CO'], vp+co_gap) + clearance(reg['V1'], vp+rules.V1_SPACE_MIN) +
            clearance(wide, rules.M1_WIDE_SPACE_MIN+vp+rules.V1_ENC_M1))
