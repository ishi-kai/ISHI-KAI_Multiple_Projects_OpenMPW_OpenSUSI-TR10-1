// Design-owned two-layer grid path search. Geometry validation is in Python.
#include <algorithm>
#include <cstdint>
#include <fstream>
#include <iostream>
...
int main(int argc,char**argv){
 if(argc!=3)return 2;
 std::ifstream f(argv[1],std::ios::binary);int32_t h[4];f.read((char*)h,sizeof h);
 int w=h[0],ny=h[1],vc=h[2],maxnodes=h[3],n=w*ny;
 ...
 while(!q.empty()){
  ...
  if(x)relax(i-1,1);if(x+1<w)relax(i+1,1);
  if(y)relax(i-w,1);if(y+1<ny)relax(i+w,1);
