...
def main():
    ...
    source_bbox = bbox_signature(source_top)
    candidate_bbox = bbox_signature(cand_top)
    same_bbox = source_ly.dbu == cand_ly.dbu and source_bbox == candidate_bbox
    size_budget = st.get("max_core_size_um")
    bbox_growth = tuple(round(a - s, 4) for a, s in zip(candidate_bbox, source_bbox))
    bbox_size = (round(candidate_bbox[2] - candidate_bbox[0], 4),
                 round(candidate_bbox[3] - candidate_bbox[1], 4))
    if size_budget is None:
        bbox_ok = same_bbox
        bbox_check_name = "bbox_unchanged"
    else:
        if (not isinstance(size_budget, (list, tuple)) or len(size_budget) != 2 or
                any(not isinstance(v, (int, float)) or v <= 0 for v in size_budget)):
            raise ValueError("MAZE_ROUTE.max_core_size_um must be a positive [width,height] pair")
        bbox_ok = (source_ly.dbu == cand_ly.dbu and
                   bbox_size[0] <= float(size_budget[0]) and
                   bbox_size[1] <= float(size_budget[1]))
        bbox_check_name = "bbox_within_explicit_size_budget"
    ...
    results["checks"].update({bbox_check_name: bbox_ok,
    ...
    if not bbox_ok:
        results["errors"].append("candidate bbox/DBU violates configured unchanged-box rule or size budget")
    ...
    results.update({"status": "ACCEPTED" if not results["errors"] and all(results["checks"].values()) else "REJECTED",
                    ...
                    "geometry": {"bbox_validation_mode": "size_budget" if size_budget is not None else "unchanged",
                                 "bbox_before_um": list(source_bbox),
                                 "bbox_after_um": list(candidate_bbox),
                                 "bbox_growth_um_by_edge": list(bbox_growth),
                                 "bbox_size_um": list(bbox_size),
                                 "max_core_size_um": list(size_budget) if size_budget is not None else None,
                                 "bbox_unchanged": same_bbox,
                                 "stdcell_hierarchy_unchanged": hierarchy_ok,
