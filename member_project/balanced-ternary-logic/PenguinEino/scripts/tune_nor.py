"""Bounded nominal NOR sizing exploration. Outputs: simulation/nor_tuning/."""
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor
import itertools,json,re,subprocess,tempfile
import numpy as np
ROOT=Path(__file__).resolve().parents[1];WORK=ROOT/'simulation/nor_tuning'
BASE=(WORK/'nor_tb.spice').read_text().split('**** begin user architecture code')[0]
BASE=re.sub(r'\n\+\s*',' ',BASE)
...
def run(cfg,tran=False,cap=10,temp=27,step=1):
 ...
 for name,w in [('XXM4',p),('XXM3',p),('XXM2',n),('XXM1',n)]:
 ...
 for i in range(1,len(ROUTE)):
  old=-max(STATES[ROUTE[i-1]]);new=-max(STATES[ROUTE[i]])
...
def dc(cfg):
 try:return run(cfg)[0]
 except RuntimeError as e:return dict(cfg=list(cfg),area=2*cfg[0]+2*cfg[1]+2*cfg[2]*cfg[3],error=999,band=999,rejected=str(e))
if __name__=='__main__':
 cfgs=list(itertools.product([13.5,20,30,40,50,60],[3.4,5,6.8,8,12,16],[2.8,4],[13,18,24,30,40]))
