"""Verify Half adder GDS against the current Xschem cell and unmodified TR-1um decks.
Run after layout generation: python3 scripts/verify_half_adder_layout.py
Reports: simulation/half_adder_layout/verify and reports/half_adder_layout.json.
"""
...
WORK=ROOT/'simulation/half_adder_layout/verify'
...
def reference():
 ...
 code,log=run(['xschem','-r','-x','--rcfile',rc,'-s','--command','set result [xschem netlist]; puts [xschem get infowindow_text]; exit $result','-o',d,ROOT/'half_adder.sch'],d,'netlist.log')
 if code or re.search(r'Error:|SKIPPING|IS MISSING',log):raise RuntimeError('Reference netlisting failed: '+log[-1000:])
 text=(d/'half_adder.spice').read_text()
...
def prepare_gui_reference(ref):
 ...
 out=ROOT/'simulation/half_adder.spice';out.parent.mkdir(parents=True,exist_ok=True)
 if out.exists() and out.read_bytes()!=Path(ref).read_bytes():
  shutil.copy2(out,out.with_name(f'half_adder.{sha(out)[:16]}.spice.bak'))
...
def lvs(gds,top,ref,d):
 ...
 code,log=run([BIN,'-b','-r',TECH/'lvs/run.lvs','-rd',f'input={gds}','-rd',f'top_cell={top}','-rd',f'report={out}','-rd',f'circuit={ref}','-rd',f'extracted={d}/half_adder.extracted'],d,'lvs.log')
 if code or not out.exists():return dict(passed=False,exit_code=code,error=log[-1500:])
...
def check(gds,top='half_adder'):
 ...
 result=dict(gds=str(gds),gds_sha256=sha(gds),schematic_sha256=sha(ROOT/'half_adder.sch'),reference_sha256=sha(ref),top=top,pdk=str(PDK),rule_sha256=rules,waiver_regions=0)
 ...
 (ROOT/'reports').mkdir(exist_ok=True);(ROOT/'reports/half_adder_layout.json').write_text(json.dumps(result,indent=2)+'\n')
 print(json.dumps({k:v for k,v in result.items() if k!='rule_sha256'},indent=2))
 return result
if __name__=='__main__':
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--gds',type=Path,default=ROOT/'half_adder.gds')
