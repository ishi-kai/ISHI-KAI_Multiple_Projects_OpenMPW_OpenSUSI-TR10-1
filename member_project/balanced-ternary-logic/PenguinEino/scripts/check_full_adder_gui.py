...
try:
    ...
    drc = view.rdb(before)
    drc.save(str(ROOT / 'simulation/full_adder_gui_drc.lyrdb'))
    pya.Macro(str(TECH / 'lvs/lvs.lylvs')).run()
    lvs = view.lvsdb(0)
    assert lvs is not None, 'LVS did not create a report'
    lvs.write(str(ROOT / 'simulation/full_adder_gui_lvs.lvsdb'))
    ...
    result = dict(gds=str(ROOT / 'full_adder.gds'),
                  gds_sha256=hashlib.sha256((ROOT / 'full_adder.gds').read_bytes()).hexdigest(),
    ...
    (ROOT / 'reports/full_adder_gui.json').write_text(json.dumps(result, indent=2) + '\n')
