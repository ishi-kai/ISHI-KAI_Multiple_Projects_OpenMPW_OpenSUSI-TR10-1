...
try:
    cv = app.main_window().load_layout(str(ROOT / 'nany.gds'), 0)
    cv.cell = cv.layout().cell('nany')
    ...
    drc = view.rdb(before)
    drc.save(str(ROOT / 'simulation/nany_gui_drc.lyrdb'))
    pya.Macro(str(TECH / 'lvs/lvs.lylvs')).run()
    lvs = view.lvsdb(0)
    assert lvs is not None, 'LVS did not create a report'
    lvs.write(str(ROOT / 'simulation/nany_gui_lvs.lvsdb'))
    ...
    result = dict(gds=str(ROOT / 'nany.gds'),
                  gds_sha256=hashlib.sha256((ROOT / 'nany.gds').read_bytes()).hexdigest(),
    ...
    (ROOT / 'reports/nany_gui.json').write_text(json.dumps(result, indent=2) + '\n')
