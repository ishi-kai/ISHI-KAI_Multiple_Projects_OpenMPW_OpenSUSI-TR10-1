"""Verify NANY GDS against the current Xschem cell and unmodified TR-1um decks.
Run after layout generation: python3 scripts/verify_nany_layout.py
Reports: simulation/nany_layout/verify and reports/nany_layout.json.
"""
...
WORK=ROOT/'simulation/nany_layout/verify'
...
def reference():
 ...
 code,log=run(['xschem','-r','-x','--rcfile',rc,'-s','--command','set result [xschem netlist]; puts [xschem get infowindow_text]; exit $result','-o',d,ROOT/'nany.sch'],d,'netlist.log')
 if code or re.search(r'Error:|SKIPPING|IS MISSING',log):raise RuntimeError('Reference netlisting failed: '+log[-1000:])
 text=(d/'nany.spice').read_text()
...
def prepare_gui_reference(ref):
 ...
 out=ROOT/'simulation/nany.spice';out.parent.mkdir(parents=True,exist_ok=True)
 if out.exists() and out.read_bytes()!=Path(ref).read_bytes():
  shutil.copy2(out,out.with_name(f'nany.{sha(out)[:16]}.spice.bak'))
...
def lvs(gds,top,ref,d):
 ...
 code,log=run([BIN,'-b','-r',TECH/'lvs/run.lvs','-rd',f'input={gds}','-rd',f'top_cell={top}','-rd',f'report={out}','-rd',f'circuit={ref}','-rd',f'extracted={d}/nany.extracted'],d,'lvs.log')
 if code or not out.exists():return dict(passed=False,exit_code=code,error=log[-1500:])
...
def check(gds,top='nany'):
 ...
 result=dict(gds=str(gds),gds_sha256=sha(gds),schematic_sha256=sha(ROOT/'nany.sch'),reference_sha256=sha(ref),top=top,pdk=str(PDK),rule_sha256=rules,waiver_regions=0)
 ...
 (ROOT/'reports').mkdir(exist_ok=True);(ROOT/'reports/nany_layout.json').write_text(json.dumps(result,indent=2)+'\n')
 print(json.dumps({k:v for k,v in result.items() if k!='rule_sha256'},indent=2))
 return result
if __name__=='__main__':
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--gds',type=Path,default=ROOT/'nany.gds')
