"""Verify perimeter routing electrical equivalence and run the native extracted TB."""
...
def main():
 ...
 proof=dict(passed=True,baseline_commit=BASE,gds_sha256=logic.sha(ROOT/'mac.gds'),extracted_sha256=logic.sha(ROOT/'mac.extracted'),cells=sorted(a),dbu_um=.001,grid_um=.05,bbox_um=[bbox.left,bbox.bottom,bbox.right,bbox.top],changes='All eleven external ports at left/right allocation edges with straight outward access; original circuit and art preserved',comparison='Exact named formal ports, device terminal roles, models and parameters; all 9 electrical cells isomorphic.',scope='Device extraction, no wire RC; exhaustive results inherited only by proved electrical equivalence.')
 print('PASS: all nine electrical cells unchanged; allocation/grid pass',flush=True)
 ex.WORK=ROOT/'simulation/mac/edge_ports_extracted';base,_,_=ex.prepare();native=logic.native(base,ex.WORK);assert native['passed']
 proof['fresh_native']=native
 (ROOT/'reports/mac_edge_ports.json').write_text(json.dumps(proof,indent=2)+'\n')
 report=copy.deepcopy(old);report.pop('final_review',None);report.pop('art_equivalence',None);report.update(gds_sha256=r['gds_sha256'],extracted_sha256=logic.sha(ROOT/'mac.extracted'),cases=[native]+old['cases'][1:],edge_ports_equivalence='reports/mac_edge_ports.json')
 for c in report['cases'][1:]:c['requalified_by']='reports/mac_edge_ports.json'
