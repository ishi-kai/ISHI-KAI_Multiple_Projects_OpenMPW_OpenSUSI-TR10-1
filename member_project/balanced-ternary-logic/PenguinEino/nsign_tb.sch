...
F {}
E {}
T {NSIGN / y = -sign(a+b) / DC + TRANSIENT} 40 -160 0 0 0.4 0.4 {}
T {DUT: nsign.sym -> nsign.sch / dimensions and wiring live in the cell} 40 -100 0 0 0.25 0.25 {}
T {27 C / V+ = +5 V / V- = -5 V / external Cload = 10 fF} 40 -55 0 0 0.25 0.25 {}
C {/home/ishi-kai/balanced-ternary-logic/nsign.sym} 400 140 0 0 {name=x1}
...
T {V0 -> GND (0 V reference); ITEST = 0 except for DC load probes.} 40 320 0 0 0.25 0.25 {}
...
alter VB -5
dc VA -5 5 0.025
plot v(a) v(vout) ylimit -5.5 5.5 title 'NSIGN DC: B=-5 V'
wrdata nsign_dc_b_neg.txt v(a) v(vout)
...
alter VB 0
dc VA -5 5 0.025
plot v(a) v(vout) ylimit -5.5 5.5 title 'NSIGN DC: B=0 V'
wrdata nsign_dc_b_zero.txt v(a) v(vout)
...
dc VA -5 5 0.025
plot v(a) v(vout) ylimit -5.5 5.5 title 'NSIGN DC: B=5 V'
wrdata nsign_dc_b_pos.txt v(a) v(vout)
...
reset
tran 0.2n 6600n
plot v(a) v(b) v(vout) title 'NSIGN: nine input pairs and mixed-rail history'
plot v(a) v(b) v(vout) xlimit 1.8u 6.6u title 'NSIGN: same mixed inputs after opposite precharges'
wrdata nsign_tran.txt v(a) v(b) v(vout)
