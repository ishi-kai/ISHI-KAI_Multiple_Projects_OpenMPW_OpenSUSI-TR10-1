...
F {}
E {}
T {PTI / 2-MOS / DC + ALL SIX TRANSITIONS} 40 -160 0 0 0.4 0.4 {}
T {DUT: pti.sym -> pti.sch / dimensions and wiring live in the cell} 40 -100 0 0 0.25 0.25 {}
T {27 C / V+ = +5 V / V- = -5 V / external Cload = 10 fF} 40 -55 0 0 0.25 0.25 {}
C {/home/ishi-kai/balanced-ternary-logic/pti.sym} 400 140 0 0 {name=x1}
...
plot v(vin) v(vout) title 'PTI: DC transfer'
wrdata pti_dc.txt v(vin) v(vout)
reset
tran 0.05n 1400n
let expected=5-10*(v(vin)>2.5)
plot v(vin) v(vout) expected title 'PTI: all six input transitions'
wrdata pti_tran.txt v(vin) v(vout)
...
if const.failures = 0
echo PASS: PTI DC bands and transient samples
else
echo FAIL: PTI samples outside +/-0.5 V
end
