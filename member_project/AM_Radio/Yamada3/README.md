...
OTAとコンデンサで構成されたLPFの後段には、次に接続する回路のインピーダンスによってLPFの特性が影響を受けにくいように、バッファを接続します。
バッファはOPAMPを使用して、ボルテージフォロワを構成します。

### AC解析

理想的なLPFを理想素子（電圧制御電流源）を使用して実装した場合を上(IDEAL)、実際のLPFをCMOSトランジスタを使用して実装した場合は下(REAL)に示します。結果のグラフは赤い線が、理想素子(IDEAL)、黄緑色の線がCMOSトランジスタを使用して実装した場合(REAL)を示しています。
理想素子の結果に実際の実装がなるべく近づくように、トランジスタのサイズW,Lの調整をします。
...
結果のグラフは赤い線が、理想素子(IDEAL)、黄緑色の線がCMOSトランジスタを使用して実装した場合(REAL)を示しています。
OTAのDC解析は入力電圧として有効な範囲を確認するの主な目的なので ゲイン調整は、LPFのAC解析の結果を優先し、OTA単体のDC解析はゲインは、厳密に合わせこみません。
...
LPFのバッファはOPAMPをボルテージフォロワ構成で使用しています。電圧のゲインは１倍で入力電圧をそのまま出力します。出力先にインピーダンスの低い回路をつかってLPFの特性が変わらないようにインピーダンス変換することが主な目的です。

### DC解析

[tb_buffer_dc.sch](xschem/tb_buffer_dc.sch)
![tb_buffer_dc](image/tb_buffer_dc.png)
...
AMラジオチップ用のメンバーロゴを作成しました。

[AMRadioMember.gds](klayout/AMRadioMember.gds)  
![AMRadioMember](image/AMRadioMember.png)

## 理想ダイオード

TR10のダイオードモデルDPがうまく動かないのでMP(PMOS)をダイオード接続して使用しました。正転アンプで入力インピーダンスが高いので前段との接続はしやすいかもしれません。OPAMPはVCVSによる理想形ですが出力インピーダンスが多少存在しないとこの回路は動かないので50Ωを強制的に入れてあります。単体ダイオードとの比較も示していて、確かに理想ダイオードにはなっています。ここにLPF追加すれば音声になると思います。

[tb_ideal_diode.sch](xschem/tb_ideal_diode.sch)
![tb_ideal_diode](image/tb_ideal_diode.png)
