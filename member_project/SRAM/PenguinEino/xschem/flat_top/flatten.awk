...
}
{
 ...
 if($0 ~ /^\**\.SUBCKT/ && first_subckt) {
   # Preserve an enabled top-level subcircuit; **.subckt denotes a testbench.
   preserve_top = ($0 ~ /^\.SUBCKT/)
   topcell=$2
   sub(/^\*\*/,"",$0)
 }
...
END{
 ...
 if(preserve_top) print a[subckt[topcell,"first"]]
 expand(topcell,"","","")
 if(preserve_top) print ".ENDS " topcell
 else print ".end"
 # parameters:
