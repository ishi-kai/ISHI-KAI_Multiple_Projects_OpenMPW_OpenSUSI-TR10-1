#!/usr/bin/env python3
"""Smoke test the compact 512-bit transistor macro with real serial peripherals.
...
import numpy as np
sys.path.insert(0,str(Path(__file__).resolve().parent.parent/'sram_macro_study'))
from macro_model import digital
HERE=Path(__file__).resolve().parent;ROOT=HERE.parents[1]
sys.path.insert(0,str(ROOT/'scripts'))

PDK=Path('/home/ishi-kai/pdk/TR-1um')
WORK=ROOT/'build/sram_compact/full_macro'


def run(command,work,log_name):
    # Keep a live log so a long simulation can be inspected without restarting.
    with (work/log_name).open('w') as stream:
        result=subprocess.run(list(map(str,command)),cwd=work,stdout=stream,stderr=subprocess.STDOUT)
    text=(work/log_name).read_text()
    if result.returncode:raise RuntimeError(f'{log_name} failed with {result.returncode}')
    return text


def macro(rb,cb):
    rows,cols=2**rb,2**cb;c,_,_=digital(rb,cb)
    top=f'compact_strap8_{rows}x{cols}'
    extracted=(ROOT/f'build/sram_compact/{top}.extracted').read_text()
    flat=re.sub(r'\n\+\s*',' ',extracted)
...
def main():
    ...
    match=next(c for c in checks if c['top']=='compact_strap8_16x32')
    assert match['drc']['pass'] and match['lvs']['pass']
    assert match['gds_sha256']==hashlib.sha256((HERE/'compact.gds').read_bytes()).hexdigest()
    results=[]
    for dims in ((4,5),):
