#!/usr/bin/env python3
"""Put the actual T4 2x2 extraction into the current serial schematic TB."""
...
OUT=ROOT/'build/sram_t4/serial'


def substitute(template,variant):
    top=variant+'_probe_2x2'
    extracted=(ROOT/'build/sram_t4'/f'{top}.extracted').read_text()
...
def main():
    ...
    for variant in ('t4_dualwl','t4_singlewl'):
    ...
    for variant,temp,cbl,cy in [('t4_dualwl',27,'10f','100f'),('t4_singlewl',27,'10f','100f'),
                              ('t4_singlewl',85,'10f','100f'),('t4_singlewl',27,'1p','1p')]:
