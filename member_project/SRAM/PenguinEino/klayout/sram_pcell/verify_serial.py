#!/usr/bin/env python3
"""Put the actual pcell 2x2 extraction into the current serial schematic TB."""
...
OUT=ROOT/'build/sram_pcell/serial'


def substitute(template,variant):
    top=variant+'_probe_2x2'
    extracted=(ROOT/'build/sram_pcell/final'/f'{top}.extracted').read_text()
...
def main():
    ...
    for variant in ('pcell',):
        match=next(r for r in checks if r['top']==variant+'_probe_2x2')
        assert match['drc']['passed'] and match['lvs']['passed']
        assert match['gds_sha256']==hashlib.sha256((HERE/'pcell.gds').read_bytes()).hexdigest()
    ...
    results=[]
    for variant,temp,cbl,cy in [('pcell',27,'10f','100f'),('pcell',85,'10f','100f'),
                              ('pcell',27,'1p','1p')]:
