...
OUT=ROOT/'build/sram_pcell/dc'
def dc_snm(variant,read):
    top='pcell_1x1'
    path=ROOT/f'build/sram_pcell/final/{top}.extracted'
...
def main():
    ...
    values=[dc_snm('pcell',read) for read in (False,True)]
    result=dict(model_sha256=hashlib.sha256((PDK/'libs.tech/spice/models/models_IP62_mos_v2.lib').read_bytes()).hexdigest(),
        extracted_sha256=hashlib.sha256((ROOT/'build/sram_pcell/final/pcell_1x1.extracted').read_bytes()).hexdigest(),
        results=values)
